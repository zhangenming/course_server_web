    ...
    <div class="list-block">
      ...
      <div class="table" v-if="items.length">
        <div class="table-head">
          ...
          <span data-label="问卷">问卷</span>
          <span data-label="用户">用户</span>
          <span data-label="提交时间">提交时间</span>
          <span data-label="总分">总分</span>
          <span data-label="标签">标签</span>
        </div>
        <div class="table-rows">
          <div class="row" v-for="it in paginatedItems" :key="it.id">
            <span data-label="ID">{{ it.id }}</span>
            <span data-label="问卷">{{ it.survey_title }} ({{ it.survey_id }})</span>
            <span data-label="用户">{{ it.user_name }} ({{ it.user_id }})</span>
            <span data-label="提交时间">{{ fmt(it.submitted_at) }}</span>
            <span data-label="总分">{{ it.total_score }}</span>
            <span data-label="标签">{{ it.label }}</span>
          </div>
        </div>
...
.table-head { 
  display: grid; 
  grid-template-columns: 80px 2fr 1.8fr 1.6fr 1fr 1fr; 
  padding: 20px 24px; 
