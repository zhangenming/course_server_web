...
const load = async () => {
  ...
  try {
    ...
    items.value = Array.isArray(data?.items) ? (data.items as Item[]) : []
    ...
    <div class="list-block">
      ...
      <div class="table" v-if="items.length">
        <div class="table-head">
          ...
          <span data-label="用户名">用户名</span>
          <span data-label="姓名">姓名</span>
          <span data-label="状态">状态</span>
          <span data-label="问卷总数">问卷总数</span>
          <span data-label="尝试次数">尝试次数</span>
        </div>
        <div class="table-rows">
          <div class="row" v-for="it in paginatedItems" :key="it.id">
            <span data-label="ID">{{ it.id }}</span>
            <span data-label="用户名">{{ it.username }}</span>
            <span data-label="姓名">{{ it.name ?? '—' }}</span>
            <span data-label="状态">
              <span class="status-badge" :class="getStatusClass(it.status)">
                {{ getStatusText(it.status) }}
              </span>
            </span>
            <span data-label="问卷总数">{{ it.questionnaires_total }}</span>
            <span data-label="尝试次数">{{ it.attempts_total }}</span>
          </div>
        </div>
...
.table-head { 
  display: grid; 
  grid-template-columns: 80px 2fr 1.6fr 1fr 1fr 1fr; 
  padding: 20px 24px; 
